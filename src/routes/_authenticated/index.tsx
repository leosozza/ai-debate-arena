  ...
  return (
    <main className="container mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link to="/_authenticated/new"><Button><Plus className="h-4 w-4 mr-2" /> Novo debate</Button></Link>
      </div>
      ...
      {!isLoading && (!data || data.length === 0) && (
        <Card className="p-12 text-center border-dashed">
          ...
          <Link to="/_authenticated/new"><Button>Criar primeiro debate</Button></Link>
        </Card>
      )}
        ...
        {data?.map((d) => (
          <Card key={d.id} className="p-4 flex items-center justify-between gap-4 hover:border-primary/50 transition">
            <button
              ...
              onClick={() => router.navigate({ to: "/_authenticated/debates/$id", params: { id: d.id } })}
            >
            ...
            <Link to="/_authenticated/debates/$id/present" params={{ id: d.id }}>
