...
// Aceita "browser" (legado) e converte para "kokoro" antes de gravar.
...
export const deleteDebate = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("debates").delete().eq("id", data.id).eq("user_id", context.userId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
const DEBATER_STAGE_RULES = `\n\nVocê está num DEBATE de TV ao vivo, NÃO em horário eleitoral. Nunca encerre como propaganda política: não peça voto, não recite slogan de campanha, não se reapresente a cada fala. Vá direto ao argumento do bloco atual, rebata o oponente quando fizer sentido, e termine a fala no próprio argumento — sem assinatura.\n\nREGRA DE BORDÃO: se a sua persona tiver um bordão identitário (ex.: "Meu nome é X!", "Brasil acima de tudo!", assinatura de nome próprio), use-o EXCLUSIVAMENTE na fase "considerações finais" / encerramento / veredito. Em respostas, réplicas, perguntas, contribuições e qualquer turno comum, JAMAIS use o bordão e JAMAIS diga seu próprio nome em primeira pessoa. Quebrar essa regra é falha grave.\n\nREGRA ANTI-REFRÃO (CRÍTICA): nunca abra duas falas com a mesma fórmula. Nunca repita frases-âncora ou slogans identitários da persona ("A tese é...", "Senhores!", "Brasil acima de tudo", "Ora,", "Como eu disse", etc.) fora do encerramento. Varie aberturas, conectores e cadência a cada turno. Em CADA fala, introduza um dado novo, um exemplo histórico inédito ou um conceito formal diferente — é PROIBIDO reciclar o argumento anterior só trocando palavras. Argumente com vocabulário rico e preciso, pensando como o personagem real pensaria, não como uma caricatura de si mesmo.`;
