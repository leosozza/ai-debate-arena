...
function Avatar({ name, src, accent }: { name: string; src: string | null; accent: CastAccent }) {
  ...
  return (
    <div className={`h-10 w-10 rounded-full overflow-hidden ring-2 ${ring} bg-muted/40 flex items-center justify-center shrink-0`}>
      {src ? <img src={src} alt={name} className="h-full w-full object-cover" /> : <span className="text-[10px] font-bold text-muted-foreground">{initials || "?"}</span>}
    </div>
  );
}
        ...
        {members.map((m) => {
          ...
          return (
            <Tag
              ...
              className={`flex items-center gap-2 rounded-lg border border-border/50 bg-background/40 px-2.5 py-1.5 text-left transition ${onMemberClick ? "hover:border-primary/60 hover:bg-background/70 cursor-pointer" : ""}`}
            >
              <Avatar name={m.name} src={m.imageUrl} accent={m.accent} />
              <div className="min-w-0">
                <div className="font-display font-semibold text-sm leading-tight truncate max-w-[120px]">{m.name}</div>
